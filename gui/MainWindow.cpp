...
MainWindow::MainWindow() {
    ...
    //create title:
    title = new QLabel(
        tr("<h2><font color=black> String Statistics </font></h2>")
    );
    ...
    QLabel *label1 = new QLabel(
        tr("<h3> Number of different occurrences: </h3>")
    );
    ...
    numOfWords = 0;
    numOfLetters = 0;
    //numberOfLetters = new QLabel(QString::number(0));
    //numberOfLetters->setFixedSize(200,30);
    //numberOfLetters->setAlignment(Qt::AlignLeft);
    //numberOfLetters->setAlignment(Qt::AlignVCenter);
    ...
    // Stretch factor Button group: 1
    mainLayout->addWidget(createExclusiveGroup(), 1);
    // Stretch factor Table: 6
    mainLayout->addWidget(wordTable, 6);
...
MainWindow::~MainWindow(){
    ...
    /* Destructor: Delete ALL the pointers */
...
void MainWindow::fillTable(){
    ...
    if (radio1->isChecked()){
        ...
        for(mit=letterMap.begin() ; mit!=letterMap.end() ; mit++) {
            ...
            QStandardItem *item2 = new QStandardItem(aux2);
            QString aux3;
            aux3.setNum(letterMap.getPercentage(mit->first), 'f', 3 );
            QStandardItem *item3 = new QStandardItem(aux3);
            ...
            //refreshing numberOfWords:
            numberOfWords->setText(QString::number(numOfLetters));
...
QGroupBox *MainWindow::createExclusiveGroup(){

    //Create radio buttons to choose between words and letters statistics:

     groupBox = new QGroupBox(tr("Choose one option:"));

     radio1 = new QRadioButton(tr("&Words"));
     radio2 = new QRadioButton(tr("&Letters"));

     //Create connections:
     QObject::connect(radio1,SIGNAL(clicked(bool)),this,SLOT(emptyFill()));
     //QObject::connect(radio1,SIGNAL(clicked(bool)),this, SLOT(resetStatistics()));
     QObject::connect(radio2,SIGNAL(clicked(bool)),this,SLOT(emptyFill()));
     //QObject::connect(radio2,SIGNAL(clicked(bool)),this,SLOT(resetStatistics()));
...
void MainWindow::handleMap(){
    ...
    /* handleMap handles wordMap. A message is sent to the standard
       error stream (stderr) if we are not able to open the file */

    try
    {
        wordMap.addFile(name);
        letterMap.addCharFile(name);
    }
    catch (int e)
    {
